package net.servokio.q4mc;

import java.util.Arrays;
import java.util.List;

public class Config {
    public static String TOKEN = "OTYxMzIwNDQ0MTU2MzE3Nzk4.Gqz9J6.2FOtQTRZdEjOsg13ylDPGVBvlzrt4M1M8dXeic";
    public static String SERVER_NAME = "Q4Bot";

    @Deprecated
    public static int GET_INFO_METHOD = 1;

    public static int ATTACK_POWER = -1;
    public static int ATTACK_TIME = 120;
    public static String ATTACK_IMAGE_URL = "https://cdn.discordapp.com/attachments/967462195409608744/982300206370537503/123.gif";
    public static String SERVOKIO_BASE_API = "https://servokio.ru/api4";
    public static List<String> WHITELIST_SERVERS = Arrays.asList("979601717215764481", "961253191620648961", "982616194198679594");
    public static String VERSION = "1.0.7";

    public static List<String> CHANGE_NICKS = Arrays.asList("Ива", "iva", "Чел какой-то", "София", "София Давалка <3", "НН Какой-тааа", "Мох", "Дебил", "Люблю хуи", "Люблю сосать писюн", "Кто сменит ник, тот крутой", "Люблю Q4");
}
